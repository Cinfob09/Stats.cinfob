...
export function MetaIntegrationPage() {
  ...
  const handleConnectFacebook = () => {
    ...
    // Assure-toi que cette URL est EXACTEMENT la même dans la console Meta
    const redirectUri = `${window.location.origin}/meta/callback`;

    const scope = [
      'public_profile', // Requis pour l'auth de base
      'pages_show_list',
    ...
    const authUrl =
      `https://www.facebook.com/v21.0/dialog/oauth?` + // v21.0 est la version 2025
      `client_id=${appId}` +
      `&redirect_uri=${encodeURIComponent(redirectUri)}` +
      `&scope=${scope}` +
      `&response_type=token` + // <--- CHANGE "code" par "token" pour une app React pure (Implicit Flow)
      `&state=${user?.id}`;
